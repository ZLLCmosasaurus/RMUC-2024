...
float accumulated_yaw_angle = 0.0f;	  // 初始化累计角度为0
float accumulated_pitch_angle = 0.0f; // 初始化累计角度为0
...
void gimbal_control_behaviour(void)
{
	...
	if (vision_mode == VISION_OFF && assist_vision_mode == ASSIST_VISION_OFF)
	{
		...
		gimbal_p.auto_aim_angle = gimbal_p.IMU_actual_angle; // 避免赋为上一次自瞄角度
		gimbal_y.auto_aim_angle = gimbal_y.IMU_actual_angle;
	...
	else // 自瞄模式开
	{
		...
		{
			// accumulated_yaw_angle += yaw_angle;
			// accumulated_pitch_angle += pitch_angle;
			// if (fabss(accumulated_yaw_angle) > 10.0f)
			// {
			// 	accumulated_yaw_angle = 10.0f;
			// }
			// if (fabss(accumulated_pitch_angle) > 5.0f)
			// {
			// 	accumulated_pitch_angle = 5.0f;
			// }
		...
		{
			// accumulated_yaw_angle = 0.0f;
			// accumulated_pitch_angle = 0.0f;
		}
...
void fly_pitch_judge(void)
{
	...
	else if (((chassis_pitch_angle <= 5) && (chassis_pitch_angle >= -5)) && fly_flag == 1) // 结束飞坡
	{
		if (last_pitch_angle1 > 0)
			gimbal_p.target_angle = pitch_angle + gimbal_p.IMU_actual_angle - 30;
		else
			gimbal_p.target_angle = pitch_angle + gimbal_p.IMU_actual_angle + 30;
		fly_flag = 0;
	}
	else if (deadline_judge(pitch_angle, 2) != 0)
	{
		if (fly_flag == 1) // 飞坡过程中的限位
		{
			...
			if (gimbal_p.target_angle <= -15 + chassis_pitch_angle)
				gimbal_p.target_angle = -15 + chassis_pitch_angle;
			if (gimbal_p.target_angle >= 30 + chassis_pitch_angle)
				gimbal_p.target_angle = 30 + chassis_pitch_angle;
		}
		else // 平地限位
		{
			gimbal_p.target_angle = pitch_angle + gimbal_p.IMU_actual_angle;
			if (gimbal_p.target_angle <= -18)
				gimbal_p.target_angle = -18;
			if (gimbal_p.target_angle >= 30)
				gimbal_p.target_angle = 30;
		}
	}

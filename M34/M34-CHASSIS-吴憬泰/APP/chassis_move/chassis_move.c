...
uint8_t fly_flag;
// 同向加速/减速
float max_d_speed_x = 30.f;
float max_d_speed_y = 30.f;
// 反向加速/减速
float max_d_speed_y_change = 20.0f;
float max_d_speed_x_change = 20.0f;
// 刹车
...
float reduction_ratio = 19.2032f; // 新轮子电机减速比
float chassis_radius = 7.7f;	  // 轮子半径    cm
...
float power_supercap_scale()
{
	float scale;
	scale = (supercap_object.percent - 0.1f)/0.9f;
	if (scale < 0)
		scale = 0.0f;
	return scale;
}
...
void chassis_power_control(void)
{
	...
	if (supercap_object.power_mode == BATTRY)
	{
	...
	if (supercap_object.power_mode == SUPER_CAP)
	{
		input_power = (supercap_object.voltage - 13.0f) * 10.0f;
	...
	if (initial_total_power > chassis_max_power) // determine if larger than max power
	{
		...
		if (supercap_object.power_mode == BATTRY)
		{
			power_scale = power_buffer_scale(remaning_buffer, total_buffer);
		...
		if (supercap_object.power_mode == SUPER_CAP)
		{
			power_scale = power_supercap_scale();
		}
...
static void power_limitation_jugement(void)
{
	...
	if (power > max_power * 2.f)
		CHASSIS_vPID_max = 5000;
	else if (power < max_power * 1.6f)
		CHASSIS_vPID_max = 8000;
	if (buffer < BUFFER_MAX * 0.25f)
		CHASSIS_vPID_max = 1900;
	if (CHASSIS_vPID_max <= 2000 && buffer >= BUFFER_MAX * 0.50f)
		CHASSIS_vPID_max = 2500;
	if (CHASSIS_vPID_max <= 2600 && buffer >= BUFFER_MAX * 0.75f)
		CHASSIS_vPID_max = 4200;
	if (CHASSIS_vPID_max <= 4500 && buffer >= BUFFER_MAX * 0.96f)
		CHASSIS_vPID_max = 8000;
}
...
static void Power_limit(void) // 新的功率限制
{
	...
	if (supercap_volt < 13)
		supercap_bettery_flag = 0;
	if (supercap_volt > 19)
		supercap_bettery_flag = 1;
	if ((buffer > 60 && buffer <= 250))
		fly_buffer_flag = 1; // 飞坡增益触发
	else
		fly_buffer_flag = 0;

	// 根据电容组电压线性改变限制功率

	max_power_k = (float)(supercap_volt - 12) / (22 - 14);
	if (max_power_k < 0)
		max_power_k = 0;
	max_power += (120 - max_power) * max_power_k;

	if ((Power_in > max_power) && (fly_buffer_flag == 0)) // 超功率并且超电充电中并且没有触发飞坡增益
	{
...
static void speed_optimize(void)
{
	...
	{
		if ((last_xspeed * chassis_control_order.vx_set) < 0) // 异号  反向加速
		{
			if (chassis_control_order.vx_set - last_xspeed > max_d_speed_x_change)
				real_chassis_speed.real_vx += max_d_speed_x_change;
			else if (chassis_control_order.vx_set - last_yspeed < -max_d_speed_x_change)
				real_chassis_speed.real_vx -= max_d_speed_x_change;
			else
	...
	{
		if ((last_yspeed * chassis_control_order.vy_set) < 0) // 异号  反向加速
		{
...
static float chassis_follow(void)
{
	...
	follow_pid_realize();
	if (chassis_center.pid.position_loop.apid.PID_OUT>=5.f)
	{
		chassis_center.pid.position_loop.apid.PID_OUT = 5.f;
	}
	if (chassis_center.pid.position_loop.apid.PID_OUT<=-5.f)
	{
		chassis_center.pid.position_loop.apid.PID_OUT = -5.f;
		...
		if (abs(GIMBAL_HEAD_ANGLE - chassis_center.actual_angle) > 1.0f)
			...
			return (float)chassis_center.pid.position_loop.apid.PID_OUT / 10.0f;
}
STEPSTAR step_flag;
...
void step_flag_judge(float VX_, float VY_, float LAST_VX_, float LAST_VY_)
{
	...
	{
		if (abs(VX_ - LAST_VX_) > STEP_VALUE && abs(VX_) > 100)
			step_flag = X_STEP;
		else if (abs(VY_ - LAST_VY_) > STEP_VALUE && abs(VY_) > 90)
			step_flag = Y_STEP;
		return;
	}
	if (step_flag == X_STEP)
...
static void chassis_move_mode(void)
{
	...
	wz = 0.24f * ((float)chassis_control_order.wz_set);
	//	wz=2.5;
	//	vx=yu;
	avx = vx;
	avy = vy;
	...
	case CHASSIS_NORMAL: // 正常模式   0
		chassis_control_order.last_chassis_mode = CHASSIS_NORMAL;
		break;
	// case CHASSIS_NO_FORCE:
	case CHASSIS_NO_FORCE: // 无力模式  3
		chassis_control_order.last_chassis_mode = CHASSIS_NO_FORCE;
	...
	case CHASSIS_FOLLOW: // 跟随模式   1
		chassis_control_order.last_chassis_mode = CHASSIS_FOLLOW;
	...
	case CHASSIS_SPIN: // 小陀螺模式   2
		...
		if (chassis_control_order.control_mode == KEY_OFF)
		{
			if (chassis_control_order.last_chassis_mode == CHASSIS_FOLLOW)
			{
				chassis_control_order.spin_dir *= (-1);
			}
			wz = 7.f * chassis_control_order.spin_dir;
		}
		if (chassis_control_order.control_mode == KEY_ON)
		{
			wz = 7.0f;
		}
		chassis_control_order.last_chassis_mode = CHASSIS_SPIN;
		//
		break;
	default:
		break;
